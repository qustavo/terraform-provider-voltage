...
func (c *Client) CreateNode(ctx context.Context, m *nodeModel) error {
	body := voltage.PostNodeCreateJSONRequestBody{
		...
		Settings: voltage.NodeSettings{
			Autopilot: toPtr(m.Settings.AutoPilot.ValueBool()),
			Grpc:      toPtr(m.Settings.Grpc.ValueBool()),
			Rest:      toPtr(m.Settings.Rest.ValueBool()),
			Keysend:   toPtr(m.Settings.Keysend.ValueBool()),
			Whitelist: toPtr(each(
				m.Settings.Whitelist, func(w types.String) string { return w.ValueString() },
			)),
			Alias: toPtr(m.Settings.Alias.ValueString()),
			Color: toPtr(m.Settings.Color.ValueString()),
		},
	}
